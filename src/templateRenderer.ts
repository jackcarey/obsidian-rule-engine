...
export async function renderTemplate(
	...
	if (frontmatter && frontmatter.position) {
		const position = frontmatter.position as {
			start?: { offset: number };
			end?: { offset: number };
		};
		if (
	...
	const resolveValue = (
		...
		if (isFileProperty) {
			if (key === "name") value = file.name;
				...
				value = file.stat.ctime; // Timestamp for dates
		...
		if (value === undefined && frontmatter && frontmatter[key] !== undefined) {
			const frontmatterValue = frontmatter[key] as
				| string
				| number
				| boolean
				| string[]
	...
	if (contentEl) {
		const sizer = contentEl.createDiv({
