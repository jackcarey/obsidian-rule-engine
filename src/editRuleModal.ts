...
function renderFilterSummary(container: HTMLElement | DocumentFragment, plugin: ObsidianRuleEnginePlugin, group: FilterGroup): void {
    ...
    if (group.conditions.length === 0) {
        wrapper.createDiv({ cls: "ore-filter-summary-empty", text: "No filters — matches everything" });
        return;
    }
    ...
    for (const condition of group.conditions) {
        ...
        if (condition.type === "group") {
            ...
            item.setText(`${plugin.getPropertyLabel(condition.field)} ${condition.operator}${value}`);
        }
    }
}
...
export class EditRuleModal extends Modal {
    ...
    onOpen() {
        ...
        const buildFilterSummary = () => createFragment(frag => renderFilterSummary(frag, this.plugin, this.rule.filterGroup));
        const filterGroup = new SettingGroup(contentEl).setHeading("Filters");
        filterGroup.addSetting(setting => {
            filterSetting = setting;
            setting
                .setDesc(buildFilterSummary())
                .addButton(btn => {
                    btn.setIcon("pencil")
        ...
        const commandsContainer = commandsGroup.listEl.createEl("ol", { cls: "ore-parent-commands-container" });
                    ...
                    setting.addExtraButton(btn => {
                        btn.setIcon('settings')
                            ...
                            .onClick(() => {
                                ...
                                new CommandSettingsModal(this.app, shortId, cmdWithSetup.name, settingCallback, currentConfig, saveFn).open();
                            });
                    });
                }
        ...
        const templateGroup = new SettingGroup(contentEl).setHeading("HTML template");
        ...
        new ButtonComponent(buttonContainer)
            ...
            .setTooltip("Save a copy of this rule (with its current unsaved edits) as a new rule")
            .onClick(async () => {
