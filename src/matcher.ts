...
const momentFn = moment as unknown as (value?: string | number | Date) => {
	subtract: (n: number, u: string) => { valueOf: () => number };
	add: (n: number, u: string) => { valueOf: () => number };
	valueOf: () => number;
	isValid: () => boolean;
};
...
function parseRelativeValue(
	...
	if (isNaN(amount) || amount <= 0 || !RELATIVE_DATE_UNITS_SET.has(unit))
		return [0, "days"];
...
export function checkRules(
	...
	log?.(
		`  group (${group.operator ?? "AND"}): ${finalResult ? "MATCH" : "no match"}`,
	);
	return finalResult;
}
...
function evaluateFilter(
		...
		switch (filter.operator) {
			...
			case "does not link to": {
				...
				const linkPaths = links
					.map((link) => {
						const resolvedPath = app.metadataCache.getFirstLinkpathDest(
							link.link,
							file.path,
						);
			...
			case "is not in folder": {
				...
				const isInFolder =
					normalizedFileFolder === normalizedTarget ||
			...
			case "does not have tag": {
				...
				const frontmatterTags = frontmatter?.tags as
					| string
					| string[]
					| undefined;
			...
			case "does not have property": {
				...
				return filter.operator === "has property"
					? !!hasProperty
					: !hasProperty;
			}
	...
	if (filter.field.startsWith("file.")) {
		...
		else if (filter.field === "file.folder")
		...
		const cache = app.metadataCache.getFileCache(file);
		const frontmatterAliases = frontmatter?.aliases as
			| string
			| string[]
			| undefined;
		const cacheAliases = cache?.frontmatter?.aliases as
			| string
			| string[]
			| undefined;
	...
	if (
		...
		if (
			filter.operator === "within past" ||
			filter.operator === "within future"
		) {
			...
			if (filter.operator === "within past") {
				return (
					targetValue >= momentFn().subtract(amt, unit).valueOf() &&
					targetValue <= nowMs
				);
			}
			return (
				targetValue >= nowMs &&
				targetValue <= momentFn().add(amt, unit).valueOf()
			);
		}
	...
	const isDateValue =
		typeof targetValue === "string" || rawTargetValue instanceof Date;
	if (
		(filter.operator === "within past" ||
			filter.operator === "within future") &&
		isDateValue &&
		...
		const [amt, unit] = parseRelativeValue(filter.value || "");
		if (!amt) return false;
		const parsed = momentFn(
			rawTargetValue instanceof Date ? rawTargetValue : String(targetValue),
		);
		...
		if (filter.operator === "within past") {
			return (
				targetMs >= momentFn().subtract(amt, unit).valueOf() &&
				targetMs <= nowMs
			);
		}
	...
	if (Array.isArray(targetValue)) {
		...
		switch (filter.operator) {
			...
			case "is":
			case "is not": {
				const match = targetArray.some(
					(v: string | number | boolean) => toString(v) === filterValue,
				);
				return filter.operator === "is" ? match : !match;
			}
			case "contains":
			case "does not contain": {
				const match = targetArray.some((v: string | number | boolean) =>
					toString(v).includes(filterValue),
				);
			...
			case "does not contain any of": {
				...
				if (filterValues.length === 0)
					return filter.operator === "does not contain any of";
				// Check if any filter value matches any target value
				const match = filterValues.some((filterVal) =>
					targetArray.some((v: string | number | boolean) =>
						toString(v).includes(filterVal),
					),
				);
			...
			case "does not contain all of": {
				...
				const filterValues = (filter.value || "")
					...
					.filter((v) => v.length > 0);
				if (filterValues.length === 0)
					return filter.operator === "does not contain all of";
		...
		switch (filter.operator) {
			...
			case "does not contain any of": {
				...
				const match = filterValues.some((filterVal) =>
					toString(targetScalar).includes(filterVal),
				);
				return filter.operator === "contains any of" ? match : !match;
			}
			case "contains all of":
