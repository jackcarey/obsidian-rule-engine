import { test, openPluginSettings, openEditRuleModal, expect } from "./fixtures";
...
test("visual: command picker combobox opens as a compact anchored dropdown", async ({ page }) => {
  const settingsPage = await openPluginSettings(page, "Rule Engine");
  await openEditRuleModal(settingsPage, 0);

  const commandsRow = settingsPage.locator(".setting-item", { hasText: "Commands" });
  await commandsRow.locator("button", { hasText: "Add" }).click();
  await settingsPage.waitForTimeout(200);
  const commandRowButton = settingsPage.locator(".ore-command-id-list-item button").first();
  await commandRowButton.click();
  await settingsPage.waitForSelector(".ore-suggestion-container", { timeout: 5000 });
  await settingsPage.waitForTimeout(300);

  await expect(settingsPage.locator(".ore-suggestion-container")).toHaveScreenshot("combobox-open.png");

  // Typing exercises the clear-button show/hide toggle.
  await settingsPage.keyboard.type("process");
  await settingsPage.waitForTimeout(200);
  await expect(settingsPage.locator(".ore-suggestion-container")).toHaveScreenshot("combobox-typed.png");
...
test("visual: a normal modal isn't left transparent after the combobox reuses modal-container", async ({ page }) => {
  const settingsPage = await openPluginSettings(page, "Rule Engine");
  await openEditRuleModal(settingsPage, 0);
  ...
  await commandsRow.locator("button", { hasText: "Add" }).click();
  await settingsPage.waitForTimeout(200);
  await settingsPage.locator(".ore-command-id-list-item button").first().click();
  await settingsPage.waitForSelector(".ore-suggestion-container", { timeout: 5000 });
  await settingsPage.keyboard.press("Escape");
  await settingsPage.waitForTimeout(200);
  await settingsPage.locator(".ore-edit-rule-modal button", { hasText: "Cancel" }).click();
  ...
  await expect(settingsPage.locator(".modal-container")).toHaveScreenshot("modal-after-combobox-reuse.png");
